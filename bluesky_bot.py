...
def is_probably_valid_image_url(img_url):
    ...
    blocked_terms = ["avatar", "logo", "icon", "favicon", "sprite", "pixel", "tracking", "spacer", "blank", "transparent", "gravatar"]
...
def get_core_image_name(img_url):
    """Extrai apenas o nome final do arquivo para evitar fotos duplicadas por causa de links diferentes."""
    if not img_url:
        return ""
...
def extract_image_urls_from_html(html_content, base_url):
    ...
    for tag in img_tags:
        attr_patterns = [r'\ssrc=["\']([^"\']+)["\']', r'\sdata-src=["\']([^"\']+)["\']', r'\sdata-original=["\']([^"\']+)["\']', r'\sdata-lazy-src=["\']([^"\']+)["\']']
        ...
        srcset_patterns = [r'\ssrcset=["\']([^"\']+)["\']', r'\sdata-srcset=["\']([^"\']+)["\']']
...
def checar_threads_token_avancado():
    ...
    try:
        ...
        data = response.json()
        if response.status_code != 200:
            print(f"Falha no debug_token: {data}")
            return None
        token_data = data.get("data", {})
        print(f"Resultado do debug_token: Válido={token_data.get('is_valid')}")
        return token_data
    except Exception as e:
        ...
        if len(image_bytes) > 1500000:
            ...
            try:
                ...
                if img.mode in ("RGBA", "P"):
                    img = img.convert("RGB")
                output = BytesIO()
                img.thumbnail((1200, 1200)) # Redimensiona limitando proporção
                img.save(output, format="JPEG", quality=85)
                image_bytes = output.getvalue()
                print(f"Imagem otimizada para {len(image_bytes)} bytes.")
            except Exception as e:
...
def criar_primeiro_post_threads_com_imagens(short_desc, images_to_post):
    ...
    if len(images) >= 2:
        ...
        for img_url in images:
            child_payload = {'media_type': 'IMAGE', 'image_url': img_url, 'is_carousel_item': 'true', 'access_token': THREADS_TOKEN}
            child_id = criar_container_threads(child_payload)
            if not child_id:
                return None
            child_container_ids.append(child_id)
            print(f"Threads: container de carrossel criado.")

        print(f"Threads: aguardando {THREADS_PROCESSING_WAIT}s para os itens do carrossel serem processados pela Meta...")
        time.sleep(THREADS_PROCESSING_WAIT)

        carousel_payload = {'media_type': 'CAROUSEL', 'children': ','.join(child_container_ids), 'text': short_desc, 'access_token': THREADS_TOKEN}
        carousel_container_id = criar_container_threads(carousel_payload)
        if not carousel_container_id:
            return None
        print(f"Threads: carrossel montado. Aguardando {THREADS_PROCESSING_WAIT}s para publicação...")
        time.sleep(THREADS_PROCESSING_WAIT)
        return publicar_container_threads(carousel_container_id)

    if len(images) == 1:
        print("Threads: criando post com 1 imagem.")
        image_payload = {'media_type': 'IMAGE', 'image_url': images[0], 'text': short_desc, 'access_token': THREADS_TOKEN}
    ...
    print("Threads: apenas texto.")
    text_payload = {'media_type': 'TEXT', 'text': short_desc, 'access_token': THREADS_TOKEN}
...
def criar_segundo_post_threads_com_link_preview(url, reply_to_id):
    ...
    reply_payload = {'media_type': 'TEXT', 'text': second_text, 'reply_to_id': reply_to_id, 'link_attachment': url, 'access_token': THREADS_TOKEN}
    reply_container_id = criar_container_threads(reply_payload)
    if not reply_container_id:
        return None
    print(f"Threads: resposta criada. Aguardando {THREADS_PROCESSING_WAIT}s...")
    time.sleep(THREADS_PROCESSING_WAIT)
...
def post_to_threads(title, short_desc, url, images_to_post):
    ...
    try:
        ...
        if not first_post_id:
            return False
        print(f"Threads: primeiro post publicado (ID: {first_post_id})")

        second_post_id = criar_segundo_post_threads_com_link_preview(url, first_post_id)
        if not second_post_id:
            return False
        print(f"Threads: resposta publicada (ID: {second_post_id})")
        return True
    except Exception as e:
...
def main():
    ...
    if not is_time_allowed():
        print("Fora do horário (10h às 22h).")
        return
    ...
    if already_bsky and already_threads:
        print("O post mais recente já foi publicado.")
        return
    ...
    print(f"Imagens finais filtradas: {len(all_images)}. Selecionadas: {len(images_to_post)}")

    sucesso_bsky = False
    sucesso_threads = False

    if already_bsky:
        print("\nBluesky: já publicado.")
    else:
    ...
    print("\nResumo da execução:")
    print(f"Bluesky: {sucesso_bsky} | Threads: {sucesso_threads}")
